// ShadowGenVertProgGL4.metal
// Translated from GLSL/ShadowGenVertProgGL4.glsl
// Modern GL4 shadow generation vertex shader with bone animation

#include <metal_stdlib>
using namespace metal;

// Reuse Transform and quaternion math from ModelVertProgGL4.metal
struct Transform {
    float4 quat;
    float4 trSc;
};
...
struct VertexIn {
    ...
    uint3 bonesInfo [[attribute(5)]];
};

struct InstanceData {
    uint4 instData [[attribute(6)]];
};
...
// Quaternion math (same as ModelVertProgGL4)
...
Transform ApplyTransformT(Transform parentTra, Transform childTra) {
    ...
    result.trSc = float4(parentTra.trSc.xyz + RotateByQuaternion(parentTra.quat, parentTra.trSc.w * childTra.trSc.xyz),
...
float4 SLerp(float4 qa, float4 qb, float t) {
    ...
    qb *= s;
    cosHalfTheta *= s;
    if (cosHalfTheta >= 0.995) return normalize(mix(qa, qb, t));
    float halfTheta = acos(cosHalfTheta);
    return normalize(qa * sin((1.0 - t) * halfTheta) + qb * sin(t * halfTheta));
}
...
vertex VertexOut shadowGenVertProgGL4(
    ...
    constant Transform* transforms [[buffer(3)]])
{
    ...
    float4 msPosition = ApplyTransform4(tx, piecePos);
    float3 msNormal = ApplyTransform4(tx, normal4).xyz;
    ...
    if (weights[0] < 1.0) {
        ...
        for (uint bi = 1; bi < 3; ++bi) {
            ...
            boneTx.trSc.w *= float(boneTx.trSc.w > 0.0);
    ...
    float4 worldPos = ApplyTransform4(txWorld, msPosition);
    txWorld.trSc = float4(0, 0, 0, 1);
    float3 worldNormal = ApplyTransform4(txWorld, float4(msNormal, 0.0)).xyz;
    ...
    // Shadow camera transform
    float4 lightVertexPos = matrices.shadowView * worldPos;
    float3 lightVertexNormal = normalize(float3x3(matrices.shadowView[0].xyz,
                                                   matrices.shadowView[1].xyz,
                                                   matrices.shadowView[2].xyz) * worldNormal);
    ...
    out.position = matrices.shadowProj * lightVertexPos;
