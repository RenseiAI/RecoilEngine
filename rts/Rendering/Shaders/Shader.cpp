...
namespace Shader {
	...
	bool IProgramObject::RemoveShaderObject(unsigned int soType) {
		RECOIL_DETAILED_TRACY_ZONE;
	...
	{
		RECOIL_DETAILED_TRACY_ZONE;
		// TODO(RHI): LuaMatTexture::Bind() calls GL directly. glActiveTexture
		// required here until LuaMatTexture is ported to use IRHITexture.
	...
	void GLSLProgramObject::SetUniform1v(UniformState* uState, int count, const int*   v) { assert(IsBound()); if (uState->Set2v(v)) glUniform1iv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform1v(UniformState* uState, int count, const float* v) { assert(IsBound()); if (uState->Set2v(v)) glUniform1fv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform2v(UniformState* uState, int count, const int*   v) { assert(IsBound()); if (uState->Set2v(v)) glUniform2iv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform2v(UniformState* uState, int count, const float* v) { assert(IsBound()); if (uState->Set2v(v)) glUniform2fv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform3v(UniformState* uState, int count, const int*   v) { assert(IsBound()); if (uState->Set3v(v)) glUniform3iv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform3v(UniformState* uState, int count, const float* v) { assert(IsBound()); if (uState->Set3v(v)) glUniform3fv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform4v(UniformState* uState, int count, const int*   v) { assert(IsBound()); if (uState->Set4v(v)) glUniform4iv(uState->GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform4v(UniformState* uState, int count, const float* v) { assert(IsBound()); if (uState->Set4v(v)) glUniform4fv(uState->GetLocation(), count, v); }
	...
	void GLSLProgramObject::SetUniform1iv(int idx, const int count, const int*   v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set2v(v)) glUniform2iv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform2iv(int idx, const int count, const int*   v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set2v(v)) glUniform2iv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform3iv(int idx, const int count, const int*   v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set3v(v)) glUniform3iv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform4iv(int idx, const int count, const int*   v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set4v(v)) glUniform4iv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform1fv(int idx, const int count, const float* v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set2v(v)) glUniform2fv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform2fv(int idx, const int count, const float* v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set2v(v)) glUniform2fv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform3fv(int idx, const int count, const float* v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set3v(v)) glUniform3fv(it->second.GetLocation(), count, v); }
	void GLSLProgramObject::SetUniform4fv(int idx, const int count, const float* v) { assert(IsBound()); auto it = uniformStates.find(uniformLocs[idx]); if (it != uniformStates.end() && it->second.Set4v(v)) glUniform4fv(it->second.GetLocation(), count, v); }
